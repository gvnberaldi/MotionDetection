...
def test_network(category, opt):
    ...
    pretrained_dict = torch.load(best_model_path, map_location=torch.device('cpu'))['model_state_dict']
    ...
    FP = 0
    TN = 0
    for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc="Testing Images"):
        ...
        if opt.save_changemap:
            ...
            vutils.save_image(fake.data, os.path.join(save_path,'%d_predicted_gt.png'%i) , normalize=True)
        ...
        FP += fp
        i += 1
        #print("\r", category, 'testing {}th images'.format(i), end='', flush=True)
...
def main(opt):
    tcfg = cfg.CONFIGS[opt.train_cfg]  
    categorys = [opt.sub_category]  # Modify this to change the test data for the output video
    out = [[] for _ in range(5)]
    cm = [[] for _ in range(4)]
    for category in categorys:
        # out0, out1 = test_network(category, opt)
        test_network(category, opt)
        '''
        for i in range(5):
            out[i].append(out0[i])
        for i in range(4):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--net_cfg', type=str, default='SViT_E1_D1_32', help='choose a TransCD model')
    parser.add_argument(
        '--train_cfg', type=str, default='CDNet_2014', help='CDNet_2014 or VL_CMU_CD')
    parser.add_argument(
        '--save_changemap', default=True)
    parser.add_argument(
        '--category', type=str, default='highway', help='Choose a category for the test')
